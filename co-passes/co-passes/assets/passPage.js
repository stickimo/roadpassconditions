import { getPrimaryPass, getPassById } from './passes.js';
import { onReady, formatElevation, createList, formatTime } from './app.js';
...
function setSnapshotSubtitle(passData) {
  ...
  if (subtitleNode) {
    subtitleNode.textContent = `Live weather snapshot for ${passData.name}.`;
  }
}
...
const ACTIONS = {
  travel: {
    'Very Low': 'Use normal winter caution and re-check soon.',
    Low: 'Use normal winter caution and re-check soon.',
    Medium: 'Slow speeds, build margin into your ETA, and stay alert for changes.',
    'Medium-High': 'Slow speeds, build margin into your ETA, and stay alert for changes.',
    High: 'Strongly consider delaying or rerouting until conditions ease.',
    'Very High': 'Strongly consider delaying or rerouting until conditions ease.',
  },
  traction: {
    'Very Low': 'Winter tires should be fine; keep AWD engaged if available.',
    Low: 'Winter tires should be fine; keep AWD engaged if available.',
    Medium: 'Prepare chains/traction devices and expect slick grades.',
    'Medium-High': 'Prepare chains/traction devices and expect slick grades.',
    High: 'Chains or auto socks likely required; have traction aids ready now.',
    'Very High': 'Chains or auto socks likely required; have traction aids ready now.',
  },
  closure: {
    'Very Low': 'Closures unlikely shortly, but keep monitoring updates.',
...
function computeSignals(weatherData) {
  ...
  return {
    precipProbability: extractPrecipProbability(weatherData.currentSummary),
...
function travelInference(signals) {
  let score = 10;
  const reasons = [];
  ...
  if (signals.heavySnowFlag) {
    ...
    reasons.push('No snow wording in immediate forecast.');
  }

  if (signals.windGust >= 45) {
    score += 20;
    reasons.push(`Gusts near ${signals.windGust} mph.`);
  } else if (signals.windSpeed >= 30) {
    score += 12;
    reasons.push(`Steady winds around ${signals.windSpeed} mph.`);
  } else {
...
function tractionInference(signals) {
  let score = 5;
  const reasons = [];
  ...
  if (signals.temperature != null) {
    ...
    reasons.push('Temperature not reported; assume typical winter slickness.');
  }
...
function closureInference(signals) {
  let score = 5;
  const reasons = [];
...
function buildInference(weatherData) {
  ...
  return {
    travel: travelInference(signals),
    traction: tractionInference(signals),
    closure: closureInference(signals),
  };
}
...
function buildConditionsRows({ passData, weatherData, gustMph }) {
  ...
  const precipProbability = extractPrecipProbability(weatherData?.currentSummary);
...
function renderWeather(weatherData, errorMessage = '') {
  ...
  precipNode.textContent =
    precipProbability != null
      ? `Precip probability: ${precipProbability}% (from NWS summary)`
      : 'Precip probability: not stated in latest summary.';
